...
import reportsFakerFactory from "./reportsFakerFactory";
...
const ReportsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = reportsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("reports").create(elem));
...
export default connect(null, mapDispatch)(ReportsFakerDialogComponent);
